...
export default function SortModal({ open, onClose, sortOption, onSortChange, dropdown }: SortModalProps) {
  if (!open) return null
  ...
  const content = (
    <div className={styles.sortModal} onClick={(e) => e.stopPropagation()}>
      ...
      <div className={styles.sortFooter}>
        <button className={styles.sortApplyBtn} onClick={onClose}>Apply</button>
      </div>
    </div>
  )
