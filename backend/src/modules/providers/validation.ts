...
import { emailSchema, myanmarPhoneSchema, usernameSchema } from "../users/validation";
import {
...
export const updateProviderProfileSchema = z.object({
  body: z
    ...
    .superRefine((value, ctx) => {
      if (value.serviceAreas && !value.location) {
        ctx.addIssue({
          code: "custom",
          path: ["serviceAreas"],
          message: "Service areas can only be updated with provider location",
        });
      }
    }),
});
